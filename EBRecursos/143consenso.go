...
var addrs = []string{"localhost:8001",
	"localhost:8002",
	"localhost:8003"}
...
func server() {
	if ln, err := net.Listen("tcp", localAddr); err != nil {
		log.Panicln("No se puede iniciar ", localAddr)
	} else {
		defer ln.Close()
		fmt.Println("Se encontro ", localAddr)
		for {
			if conn, err := ln.Accept(); err != nil {
				log.Println("No permitido", conn.RemoteAddr())
			} else {
				go handle(conn)
			}
		}
...
func handle(conn net.Conn) {
	...
	if err := dec.Decode(&msg); err != nil {
		log.Println("No se entiende ", conn.RemoteAddr())
	} else {
		fmt.Println(msg)
		switch msg.Code {
		case cnum:
